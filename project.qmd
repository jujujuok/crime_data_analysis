...
ggplot(top_categories, aes(x = "", y = count, fill = Category)) +
...
# library(plotly)
# 
# mocodes.gang <- mocodes.df%>% filter(str_detect(mocodes.df$Beschreibung, "[gG]ang"))
# mocodes.gang
# 
# crimes.per_area <- crimes.df %>%
#   group_by(`AREA NAME`, `Mocodes`)
# crimes.per_area
# 
# crimes.per_area.gang <- crimes.per_area %>%
#   inner_join(mocodes.gang, by = c("Mocodes" = "Code"))
#   group_by(`AREA NAME`, GangType) %>%
#   summarise(count = n())
# crimes.per_area.gang
# 
# crimes.per_area.gang <- crimes.per_area %>%
#   inner_join(mocodes.gang, by = c("Mocodes" = "Code")) %>% # join Mocode ! mocodes type = list
#   group_by(`AREA NAME`, Mocodes) %>%
#   summarise(count = n())
# 
# crime_percentages <- inner_join(crimes.per_area.gang, total_gang_crimes, by = "AREA NAME") %>%
#   mutate(percentage = (count / total_gang_crimes) * 100)
# 
# 
# ggplotly(
#   ggplot(crime_percentages, aes(x = `AREA NAME`, y = percentage, fill = GangType)) +
#     geom_bar(stat = "identity") +
#     labs(title = "Anteil verschiedener Gangtypen pro Gebiet",
#          x = "Gebiet",
#          y = "Anteil (%)") +
#     theme(axis.text.x = element_text(angle = 45, hjust = 1))
# )
...
- Line mit anzahl der verschiedenen straftaten über die Zeit (36 Monate)

```{r}
library(ggplot2)
library(dplyr)

# Monat und Jahr aus DATE OCC extrahieren
crimes.tib$Month_Year <- format(crimes.tib$`DATE OCC`, "%Y-%m")

# Gruppieren nach Monat und Kategorie
crimes.per_time <- crimes.tib %>%
  group_by(Month_Year, Category) %>%
  summarize(count = n(), .groups = "drop")  # Gruppierung auflösen

crimes.per_time$Month_Year <- factor(crimes.per_time$Month_Year, levels = sort(unique(crimes.per_time$Month_Year)))

ggplotly(ggplot(crimes.per_time, aes(x = Month_Year, y = count, color = Category, group = Category)) +
  geom_line() +
  labs(
    title = "Crimes over Time by Category",
    x = "Time (Month-Year)",
    y = "Number of Crimes"
  ) +
    scale_x_discrete(
    breaks = unique(crimes.per_time$Month_Year)[seq(1, length(unique(crimes.per_time$Month_Year)), by = 3)],
    labels = unique(crimes.per_time$Month_Year)[seq(1, length(unique(crimes.per_time$Month_Year)), by = 3)] 
  ) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
)
